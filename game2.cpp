...
public:
	...
	void spwanPlayer() {
		...
		entity->cCollision = new CCollision(25.00f);

		entity->cShape->circle.setOrigin(entity->cCollision->radius, entity->cCollision->radius);
	...
	Entity* spawnBullet() {
		...
		entity->cCollision = new CCollision(10.00f);
	...
	void spawnEnemy() {
		...
		entity->cTransform = new CTransform(Vec2(rand.getRandom(300.0f,1000.0f), rand.getRandom(100.0f, 600.0f)), Vec2(rand.getRandom(-2.0f,4.0f), rand.getRandom(-2.0f, 4.0f)), 0);

		entity->cShape = new CShape(25.00f, rand.getRandom(3,10), sf::Color(rand.getRandom(0,255), rand.getRandom(0, 255), rand.getRandom(0, 255)), sf::Color(rand.getRandom(0, 255), rand.getRandom(0, 255), rand.getRandom(0, 255)), 2.0f);
	...
	void collision() {
		...
		for (auto& p : entities.getEntities("player")) {



			for (auto& e : entities.getEntities("enemy")) {

				sf::Vector2f playerOrigin = p->cShape->circle.getOrigin();
				
				sf::Vector2f enemyOrigin = e->cShape->circle.getOrigin();

				sf::Vector2f diff = playerOrigin - enemyOrigin;

				float playerRadius = p->cCollision->radius;
				...
				if (distance < playerRadius + enemyRadius) {
					...
					std::cout << playerRadius + enemyRadius << std::endl;

					std::cout << "player id" << p->id() << "enemy id: " << e->id() << std::endl;
